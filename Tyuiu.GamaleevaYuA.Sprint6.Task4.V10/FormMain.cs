    ...
    public partial class FormMain : Form
    {
        ...
        private void buttonDone_GYA_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                chartFunction_GYA.Series[0].Points.Clear();
                for (int i = 0; i < len; i++)
                {

                    this.chartFunction_GYA.Series[0].Points.AddXY(startStep, valueArray[i]);
                    textBoxResult_GYA.AppendText(valueArray[i] + Environment.NewLine);
                    startStep++;
                }
        ...
        {
            MessageBox.Show("Таск 4 выполнил студент группы ИБКСб-25-1 Гамалеева Юлия Андреевна", "Сообщение", MessageBoxButtons.OK, MessageBoxIcon.Information);
        ...
        private void buttonSave_GYA_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                File.WriteAllText(path, textBoxResult_GYA.Text);
                DialogResult dialogResult = MessageBox.Show("Файл " + path + " созранен успешно!\n Открыть его в блокноте?", "Сообщение", MessageBoxButtons.YesNo, MessageBoxIcon.Information);
